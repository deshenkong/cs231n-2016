...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # scores = buf_relu * w2 + b2
    # d(loss)/d(buf_relu) = d(loss)/d(scores) * d(scores)/d(buf_relu)
    # = w2 * scores.T
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      X_batch = X[mask]
      y_batch = y[mask] 
#      print X_batch.shape, mask.shape, X.shape
#      (200L, 4L) (200L,) (5L, 4L)
      #########################################################################
      #                             END OF YOUR CODE                          #
      #########################################################################
...
#        print it, iterations_per_epoch
        # Check accuracy
        train_acc = (self.predict(X_batch) == y_batch).mean()#校验准确率
        val_acc = (self.predict(X_val) == y_val).mean()
        train_acc_history.append(train_acc)
        val_acc_history.append(val_acc)

        # Decay learning rate
        learning_rate *= learning_rate_decay#逐步减小学习率
