...
def rnn_step_backward(dnext_h, cache):
  ...
  db = np.sum((dnext_h * dtanh), axis=0)  #b也是多维,所以需要sum
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def rnn_forward(x, h0, Wx, Wh, b):#主要是计算下一个hidden的状态
  """
  ...
  cache = []
  for i in range(T):  
    #这样编写代码的原因是rnn_step_forward的x维度是N,D，需要对应
