...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    captions_in = captions[:, :-1]#去掉最后一个元素
    captions_out = captions[:, 1:]#去掉第一个元素
    #这里我没太理解，是因为要去掉in的end和out的start?
    # You'll need this 
...
#    print(features.shape, imf2hid.shape)#(25, 512) (25, 512)
#    print(captions_in.shape,W_embed.shape)#(25, 16) (1004, 256)
    word_vectors, word_cache = word_embedding_forward(captions_in, W_embed)   # [N, T, W]
#    print(word_vectors.shape) #(25, 16, 256)
    ...
    #统一计算输出值
    #print(W_vocab.shape, b_vocab.shape)#(40, 3) (3,)
  ...
  def sample(self, features, max_length=30):
    ...
    Inputs:
    - features: Array of input image features of shape (N, D).#输入N张图像特征
    - max_length: Maximum length T of generated captions. #标注的最大长度
...
#    
    ...
    captions[:, 0] = init_word #把start token加到captions前面
    print(V,W)
    for i in range(1, max_length): #计算每个caption
        onehots = np.eye(V)[captions[:, i-1]]    # [N, V]#选取caption[:,y]对应的两个列
        word_vectors = onehots.dot(W_embed)      # [N, W]#通过w_embed获得词向量    
        if self.cell_type == 'rnn':        
            h, cache = rnn_step_forward(word_vectors, h, Wx, Wh, b)    
        else:        
            h, c, cache = lstm_step_forward(word_vectors, h, c, Wx, Wh, b)
        scores = h.dot(W_vocab) + b_vocab # [N, V] #输出
