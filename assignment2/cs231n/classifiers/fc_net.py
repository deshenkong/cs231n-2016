...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    Inputs:
    - X: Array of input data of shape (N, d_1, ..., d_k) 图片个数和图片维度
...
#    pass
    ...
    #bp算法
    #bp into second layer 注意第一个参数是scores的导数
...
#初始化w,b和BN
    for i in xrange(self.num_layers):
        ...
        if self.use_batchnorm and i < len(hidden_dims):#BN层比隐藏层少1层
    ...
    if self.use_batchnorm:
      self.bn_params = [{'mode': 'train'} for i in xrange(self.num_layers - 1)]#复制多个dict
  ...
  def loss(self, X, y=None):
    ...
    #{affine - [batch norm] - relu - [dropout]} x (L - 1) - affine - softmax
    #注意一个循环只是上面的一个{}里面的内容
    #W1 b1 h[0] bn[0] out[1] cache1[0] cache2[0] cache3[0] cache4[0]
    #W2 b2 h[1] bn[1] out[2] cache1[1] cache2[1] cache3[1] cache4[1]
    #...
